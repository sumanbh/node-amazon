...
export class ShopService {
    ...
    _brandName: string[] = ['Asus', 'Acer', 'Apple', 'HP', 'Microsoft', 'Lenovo', 'Dell', 'Samsung'];
    _osName: string[] = ['Mac OS X', 'Windows 10', 'Chrome OS', 'Windows 8.1', 'Windows 7 Home'];
    _processorName: string[] = ['Intel Core i7', 'Intel Core i5', 'Intel Core i3', 'Intel Core 2', 'AMD'];
    _storageName: string[] = ['SSD', 'Hard Disk'];
    ...
    // Forgive me please. I did not know what I was doing back then
    ...
    getAllProducts(page: number, _queryParam: string, min: number, max: number, customMin: number, customMax: number): Observable<any> {
        ...
        const productUrl = `/api/shop/${page}?brand=${this._brand}&os=${this._os}&ram=${this._ram}&processor=${this._processor}&storage=${this._storage}&min=${min}&max=${max}`;  //api url
